        ...
        <div class="col-12">
            <div class="table-responsive">
                <table class="table table-bordered text-center">
                    ...
                    <tbody>
                        <tr>
                            <td rowspan="3">{{ $searchMahasiswa->nama }}</td>
                            <td rowspan="3">{{ $searchMahasiswa->nim }}</td>
                            @foreach ($searchMahasiswa->datang as $item)
                                <td colspan="5">{{ Carbon::parse($item->tanggal)->isoFormat('dddd, DD MMMM YYYY') }}
                                </td>
                            @endforeach
                        </tr>
                        <tr>
                        ...
                        <tr>
                            @if (count($searchMahasiswa->kegiatan) > 0)
                                @foreach ($searchMahasiswa->kegiatan as $item)
                                    <td>{{ $item->deskripsi }}</td>
                                @endforeach
                            @endif

                            {{-- Add empty cells for columns 4 and 5 if data is not available --}}
                            @for ($i = count($searchMahasiswa->kegiatan); $i < 5; $i++)
                                <td></td>
                            @endfor
